...
const steps = [
  ...
  },
  {
    ...
    icon: <Flag className="w-16 h-16 text-cyan-400" />,
    color: "from-cyan-500/20 to-cyan-950/20"
  },
  {
    title: "Madalya Odası",
...
export default function Onboarding({ onComplete, onSkip }: OnboardingProps) {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg hud-module overflow-hidden shadow-[0_0_50px_rgba(6,182,212,0.2)]">
        <button 
          onClick={() => onSkip(dontShowAgain)}
          className="absolute top-4 right-4 z-10 p-2 hover:bg-slate-800 rounded-sm text-slate-400 hover:text-white transition"
        >
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="p-6 bg-surface-container-low/70 space-y-6">
          <div className="flex justify-center gap-2">
            {steps.map((_, i) => (
              <div 
                key={i} 
                className={`h-1.5 transition-all duration-300 ${i === currentStep ? 'w-8 bg-cyan-400' : 'w-2 bg-slate-700'}`} 
              />
            ))}
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            >
              <div className={`w-5 h-5 flex items-center justify-center transition ${dontShowAgain ? 'bg-cyan-500' : 'bg-slate-800 group-hover:bg-cyan-500/20'}`}>
            ...
            <div className="flex gap-4">
              {currentStep > 0 && (
                <button 
                  onClick={prevStep}
                  className="p-3 rounded-sm text-slate-300 hover:bg-slate-800 transition"
                >
              ...
              <button 
                onClick={nextStep}
                className="px-6 py-3 rounded-sm bg-primary hover:bg-cyan-300 text-slate-950 font-mono font-bold uppercase tracking-widest shadow-[0_0_20px_rgba(6,182,212,0.3)] transition flex items-center gap-2"
              >
