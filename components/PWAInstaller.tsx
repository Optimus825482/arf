    ...
    <>
      ...
      {show && (deferred || manualMode) && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[100] w-[min(92vw,430px)] animate-in slide-in-from-bottom-4">
          <div className={`relative overflow-hidden border backdrop-blur-xl shadow-2xl ${isSafariGuide ? 'rounded-[28px] border-cyan-300/20 bg-slate-950/95' : 'rounded-2xl border-white/10 bg-slate-900/90'}`}>
            <div className="pointer-events-none absolute inset-0">
              <div className={`absolute -left-12 top-0 h-32 w-32 rounded-full blur-3xl ${isSafariGuide ? 'bg-cyan-400/15' : 'bg-indigo-500/20'}`} />
              <div className={`absolute right-0 top-10 h-28 w-28 rounded-full blur-3xl ${isSafariGuide ? 'bg-sky-400/15' : 'bg-fuchsia-500/20'}`} />
            ...
            {isSafariGuide ? (
              <div className="relative p-4 sm:p-5">
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-300/30 bg-gradient-to-br from-cyan-400/20 via-sky-400/15 to-slate-900 text-cyan-100 shadow-[0_0_30px_rgba(56,189,248,0.18)]">
                  ...
                  <button onClick={dismiss} aria-label="Kapat" className="rounded-xl p-2 text-slate-400 transition hover:bg-white/10 hover:text-white">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                ...
                <div className="grid gap-2.5">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-3">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full border border-cyan-300/30 bg-cyan-400/10 font-mono text-xs text-cyan-100">1</div>
                    ...
                    <div className="rounded-xl border border-cyan-300/20 bg-slate-950/80 px-2.5 py-2 text-cyan-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] motion-safe:animate-[install-share-pulse_1.8s_ease-in-out_infinite]">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-3">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full border border-sky-300/30 bg-sky-400/10 font-mono text-xs text-sky-100">2</div>
                      <div className="min-w-0">
                ...
                <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-cyan-400/15 bg-cyan-400/10 px-3 py-2.5">
                  <p className="text-[11px] uppercase tracking-[0.22em] text-cyan-100/80">Tam ekran, hizli acilis, uygulama hissi</p>
                  <button onClick={dismiss} className="rounded-xl border border-white/10 bg-white/10 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-white/15">
                    Tamam
                  </button>
                </div>
              </div>
              ...
              <div className="relative flex items-center gap-3 p-4">
                ...
                {deferred ? (
                  <button onClick={install} className="flex items-center gap-1.5 rounded-lg bg-indigo-500 px-3 py-1.5 text-sm font-medium text-white transition hover:bg-indigo-400">
                    <Download className="h-4 w-4" /> Yükle
                  </button>
                ) : null}
                <button onClick={dismiss} aria-label="Kapat" className="rounded-lg p-1.5 text-slate-400 transition hover:bg-white/10 hover:text-white">
                  <X className="h-4 w-4" />
                </button>
              </div>
            )}
