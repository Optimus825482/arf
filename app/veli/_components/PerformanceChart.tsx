...
export default function PerformanceChart({ studentData }: PerformanceChartProps) {
  if (!studentData) return null;

  const pilotId = studentData.username?.toUpperCase() || 'UNKNOWN';
  
  // Şimdilik veritabanında tarih bazlı haftalık veri tam tutulmadığı için
  // mevcut estetik yapıyı koruyoruz. İleride studentData.metrics.history üzerinden dolacak.
  const days = [
    ...
    { label: 'Paz', value: studentData.metrics?.accuracy || 95, active: true },
  ];
      ...
      <div className="relative rounded-sm overflow-hidden border border-white/5 bg-white/[0.02] hud-glass p-8 min-h-[300px] flex flex-col justify-end group">
        <div className="absolute top-0 left-0 p-4 flex gap-4 opacity-20 pointer-events-none">
          <div className="flex flex-col gap-1">
            <span className="font-mono text-[9px] text-cyan-400 font-bold uppercase">SCAN_MODE: PERFORMANCE_TRACK</span>
            <span className="font-mono text-[9px] text-cyan-400 font-bold uppercase">PILOT_ID: {studentData.username || 'UNKNOWN'}</span>
          </div>
        </div>
