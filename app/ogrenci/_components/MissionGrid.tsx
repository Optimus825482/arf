...
import { Target, Zap, Cpu, Swords, Play, Lock, MessageSquare } from 'lucide-react';
...
export default function MissionGrid({ studentData }: MissionGridProps) {
  const dailyTasks = studentData?.dailyTasks;
  const missionReport = studentData?.dailyMissionReport;
  ...
  return (
    <section className="space-y-12">
      ...
      {missionReport && (
        <div className="relative overflow-hidden p-[1px] rounded-sm bg-gradient-to-r from-cyan-400/20 via-transparent to-cyan-400/20 shadow-2xl">
          <div className="bg-[#0c0e12]/90 backdrop-blur-xl p-6 relative">
            <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-cyan-400"></div>
            <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-cyan-400"></div>
            
            <div className="flex items-start gap-5">
              <div className="w-12 h-12 bg-cyan-400/10 rounded-sm flex items-center justify-center flex-shrink-0 border border-cyan-400/20">
                <MessageSquare className="w-6 h-6 text-cyan-400" />
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <h3 className="font-headline font-black text-sm tracking-widest text-on-surface uppercase italic">GÖZCÜ RAPORU: <span className="text-cyan-400">{missionReport.title || 'GÜNLÜK ÖZET'}</span></h3>
                  <div className="h-[1px] flex-grow bg-cyan-400/20"></div>
                </div>
                <p className="font-label text-xs text-on-surface-variant/80 leading-relaxed italic">{missionReport.summary}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          {/* Locked missions placeholder */}
          <div className="relative group/card bg-[#0c0e12]/30 p-6 rounded-sm flex items-center justify-center min-h-[250px] overflow-hidden">
